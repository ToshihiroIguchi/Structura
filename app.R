...
server <- function(input, output, session) {
  ...
  output$datatable <- renderDataTable({
    req(data())
    datatable(data(), filter="top", options=list(pageLength=30, autoWidth=TRUE))
  }, server = FALSE)
  ...
  output$log_transform_ui <- renderUI({
    ...
    valid_cols<- num_cols[sapply(df[, num_cols, drop=FALSE],
  ...
  processed_data <- reactive({
    ...
    multi     <- char_cols[sapply(df[,char_cols,drop=FALSE], function(x){
      u<-unique(x); length(u)>1 && length(u)<nrow(df) })]
    if (length(multi)>0) {
      dummy_mat <- model.matrix(~ . -1, data=df[, multi, drop=FALSE])
      df <- cbind(df[, setdiff(names(df),multi), drop=FALSE],
  ...
  # ── 新規：input_table 用リアクティブ値 ─────────────────────
  input_table_data <- reactiveVal(data.frame(Input="", stringsAsFactors=FALSE))

  # input_table 表示
  output$input_table <- renderRHandsontable({
    rhandsontable(input_table_data(), rowHeaders=NULL) %>%
      hot_col("Input", readOnly=FALSE)
  })

  # Add Row ボタンで空行を追加
  observeEvent(input$add_row, {
    df <- input_table_data()
    df[nrow(df)+1, ] <- ""       # 新しい行の Input 列を空文字で
    input_table_data(df)
  })
  ...
  output$checkbox_matrix <- renderRHandsontable({
    df <- processed_data(); req(df)
    items <- if (!is.null(input$display_columns)) intersect(input$display_columns,names(df)) else names(df)
    ...
    rhandsontable(mat, readOnly=FALSE, rowHeaders=NULL) %>%
      hot_col("Item",   readOnly=TRUE) %>%
      hot_col("Symbol", readOnly=TRUE) %>%
      {
        rh <- .
        for(col in items) rh <- rh %>% hot_col(col, type="checkbox", readOnly=FALSE)
        for(i in seq_along(items)) rh <- rh %>% hot_cell(row=i, col=which(colnames(mat)==items[i]), readOnly=TRUE)
        rh
      }
  })
}
